...
def parse_args():
    ...
    parser.add_argument('--out_json', type=str, default='runs/test-orientation/yolo11n-panther_v1-pose-v1/heading_debug_metrics.json', help='Where to save per-image headings (gt, v1, v2, v2_bias_corrected).'
)
...
def _compute_v2_confidence(
    ...
    """
    Compute a confidence/quality score for the v2 heading estimation process.
    
    Evaluates multiple factors:
    - mask_fill_ratio: Percentage of ROI occupied by detected pixels
    - linearity_score: How line-like the detected points are (PCA eigenvalue ratio)
    - point_density: Number of detected pixels relative to ROI area
    - brightness_contrast: How distinct the bracket is from background
    - edge_coherence: Quality of detected edges
    
    Returns:
        dict with:
            - confidence: float in [0, 1], overall confidence score
    ...
    # 1. Mask fill ratio: how much of ROI has detected pixels
    roi_area = out_w * out_h
    mask_area = int(np.count_nonzero(mask))
    fill_ratio = mask_area / roi_area if roi_area > 0 else 0.0
    factors["mask_fill_ratio"] = min(1.0, fill_ratio * 2.0)  # scale: expect ~50% fill
    details["mask_pixel_count"] = int(mask_area)
    details["roi_area_pixels"] = int(roi_area)
    
    # 2. Linearity score: how concentrated in one direction (from PCA)
    factors["linearity"] = min(1.0, line_score / 5.0) if line_score > 0 else 0.0
    details["eigenvalue_ratio"] = float(line_score)
    
    # 3. Point density: number of pixels relative to expected bracket size
    expected_pixel_count = max(50, int(bracket_thickness_px * out_w * 0.5))
    point_density = min(1.0, mask_area / max(1, expected_pixel_count))
    factors["point_density"] = point_density
    details["expected_point_count"] = expected_pixel_count
    
    # 4. Brightness contrast: measure variance/contrast in gray image
    gray_roi = cv2.cvtColor(roi, cv2.COLOR_BGR2GRAY) if roi.ndim == 3 else roi
    roi_mean = float(gray_roi.mean())
    roi_std = float(gray_roi.std())
    # Bracket should be dark (mean < 100) with some contrast (std > 20)
    darkness_score = min(1.0, max(0.0, (100.0 - roi_mean) / 100.0))
    contrast_score = min(1.0, roi_std / 40.0)
    factors["darkness"] = darkness_score
    factors["contrast"] = contrast_score
    details["roi_mean_gray"] = float(roi_mean)
    details["roi_std_gray"] = float(roi_std)
    
    # 5. Edge coherence: how continuous/connected are the detected pixels
    # Measure: ratio of perimeter to area (lower = more solid, higher = more sparse)
    if mask_area > 0:
        edges_in_mask = cv2.Canny(mask, 10, 50)
        edge_pixels = int(np.count_nonzero(edges_in_mask))
        perimeter_ratio = edge_pixels / mask_area if mask_area > 0 else 0.0
        # Good brackets have low perimeter (more solid), so penalize high ratios
        edge_coherence = max(0.0, 1.0 - perimeter_ratio / 0.3)
        factors["edge_coherence"] = min(1.0, edge_coherence)
        details["perimeter_ratio"] = float(perimeter_ratio)
    else:
        factors["edge_coherence"] = 0.0
        details["perimeter_ratio"] = 0.0
    
    # 6. Aspect ratio check: bracket should be much longer than wide
    mask_height = np.any(mask, axis=1).sum()
    mask_width = np.any(mask, axis=0).sum()
    if mask_height > 0 and mask_width > 0:
        aspect_ratio = max(mask_width, mask_height) / min(mask_width, mask_height)
        # Expect aspect ratio > 3 for a bracket
        aspect_score = min(1.0, aspect_ratio / 5.0)
        factors["aspect_ratio"] = aspect_score
        details["detected_aspect_ratio"] = float(aspect_ratio)
    else:
        factors["aspect_ratio"] = 0.0
        details["detected_aspect_ratio"] = 0.0
    
    # Compute overall confidence as weighted mean of factors
    weights = {
        "mask_fill_ratio": 0.15,
        "linearity": 0.25,
        "point_density": 0.15,
        "darkness": 0.10,
        "contrast": 0.10,
    ...
    confidence = sum(factors[k] * weights[k] for k in weights.keys())
...
def calculate_heading_from_bracket(
    ...
    if xs.size < 30:
        conf_fail = _compute_v2_confidence(roi, mask2, np.array([]), 0.0, out_w, out_h, bracket_thickness_px)
        conf_fail["confidence"] = 0.0  # Marked as failure
        return ((None, {"roi": roi, "mask": mask2, "confidence": conf_fail}) if debug else None)

    pts = np.stack([xs.astype(np.float32), ys.astype(np.float32)], axis=1)
    d = _pca_direction(pts)
    if d is None:
        conf_fail = _compute_v2_confidence(roi, mask2, pts, 0.0, out_w, out_h, bracket_thickness_px)
        conf_fail["confidence"] = 0.0  # Marked as failure
        return ((None, {"roi": roi, "mask": mask2, "confidence": conf_fail}) if debug else None)

    # Compute line score for confidence calculation
    line_score = _eigen_ratio_line_score(pts)
    ...
    nv = math.hypot(vx, vy)
    if nv < 1e-9:
        conf_fail = _compute_v2_confidence(roi, mask2, pts, line_score, out_w, out_h, bracket_thickness_px)
        conf_fail["confidence"] = 0.0  # Marked as failure
        return ((None, {"roi": roi, "mask": mask2, "confidence": conf_fail}) if debug else None)
    vx, vy = vx / nv, vy / nv
    ...
    # Compute confidence score for this successful detection
    confidence_data = _compute_v2_confidence(roi, mask2, pts, line_score, out_w, out_h, bracket_thickness_px)

    if debug:
        dbg = {"roi": roi, "mask": mask2, "corners_img": corners_img, "M": M, "heading": heading, "confidence": confidence_data}
        return heading, dbg
    return heading, confidence_data
...
def _convert_angle_convention(angle_deg: float) -> float:
    """
    Convert from image convention (0=left, 90=down, 180=right, 270=up)
    to simulation convention (0=right, 90=down, 180=left, 270=up).
    
    Formula: target = (180 - source) % 360
    """
    return _wrap_angle_0_360_deg(180.0 - angle_deg)
...
def main():
                        ...
                        if xy_np.shape[1] >= 3:
                            ...
                            if heading_v2 is not None:
                                ...
                                if confidence_v2 is not None:
                                    conf_val = confidence_v2.get("confidence", 0.0)
                                    conf_str = f" | confidence: {conf_val:.3f}"
                                print(f"     Heading v2: {heading_v2:.1f}° (bracket PCA ⟂){conf_str}")
                            else:
                                print("     Heading v2: (failed; fallback to v1)")
                                if confidence_v2 is not None:
                                    conf_val = confidence_v2.get("confidence", 0.0)
                                    print(f"       v2 confidence: {conf_val:.3f}")
                                if dbg is not None:
                            ...
                            if gt_poses and i < len(gt_poses) and len(gt_poses[i]) >= 3:
                                ...
                                if (g0[2] > 0) and (g1[2] > 0) and (g2[2] > 0):
                                    ...
                                    # Add v3 in simulation convention (0=right, 90=down, 180=left, 270=up)
                                    if heading_v2 is not None:
                                        rec["v3_deg"] = float(_convert_angle_convention(heading_v2))
                                    else:
                                        rec["v3_deg"] = None
                                    
                                    # Add v2 confidence metrics
                                    if confidence_v2 is not None:
                                        rec["v2_confidence"] = {
                                            "overall": float(confidence_v2.get("confidence", 0.0)),
                                            "factors": confidence_v2.get("factors", {}),
                                            "details": confidence_v2.get("details", {}),
                                        }
                                    else:
                                        rec["v2_confidence"] = None
    ...
    # -------------------------
    out_payload = {
        "angle_convention_image": "0=left, 90=down, 180=right, 270=up (image coordinates), degrees",
        "angle_convention_simulation": "0=right, 90=down, 180=left, 270=up (simulation convention), degrees",
        "description": "v1 and v2 in image convention, v3 in simulation convention",
