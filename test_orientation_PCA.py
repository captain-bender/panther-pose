...
def calculate_heading_from_bracket(
    ...
    # Strong prior: bracket is a long thin bar roughly along ROI x-axis.
    # Use a horizontal structuring element to suppress non-bar blobs.
    k_len = max(9, int(round(bracket_length_px * 0.25)))   # e.g. ~15 px
    k_th  = max(3, int(round(bracket_thickness_px * 2)))   # e.g. ~5 px
    kernel_h = cv2.getStructuringElement(cv2.MORPH_RECT, (k_len, k_th))

    mask2 = cv2.morphologyEx(mask, cv2.MORPH_OPEN, kernel_h)   # keep horizontal-ish strokes
    mask2 = cv2.morphologyEx(mask2, cv2.MORPH_CLOSE, kernel_h) # reconnect bar

    # After you compute corners_img and roi/mask2, build dbg:
    dbg = {"corners_img": corners_img, "roi": roi, "mask": mask2}

    # Connected components: pick the most "bar-like" component
    ...
    for cid in range(1, num):
        ...
        if area < 40:
            continue
        aspect = w / max(1.0, h)
        # We expect a long bar: high aspect ratio, decent area
        score = aspect * area
    ...
    # Disambiguate heading direction using kp1 (same idea as your v1) :contentReference[oaicite:2]{index=2}
    if (hx * ax + hy * ay) < 0:
        hx, hy = -hx, -hy
                ...
                try:
                    ...
                    for i in range(xy_np.shape[0]):
                        ...
                        if xy_np.shape[1] >= 3:
                            ...
                            if dbg is not None and "corners_img" in dbg and dbg["corners_img"] is not None:
                                _draw_poly(overlay, dbg["corners_img"], color=(0, 255, 255), thickness=2)
