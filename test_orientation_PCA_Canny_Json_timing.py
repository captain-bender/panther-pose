...
def parse_args():
    ...
    # Inference parameters to match testing_evaluation.py
    parser.add_argument('--imgsz', type=int, default=1920, help='Inference image size (default: 1920 to match testing_evaluation.py)')
    ...
    parser.add_argument('--verbose', action='store_true', help='Enable verbose output during inference')
...
def main():
    ...
    # Initialize CUDA to avoid warm-up overhead on first inference
    try:
        import torch
        if torch.cuda.is_available():
            print("Initializing CUDA...")
            torch.cuda.init()
            # Run a dummy inference to warm up the GPU with same parameters as main run
            dummy_path = Path(args.dataset) / args.split / 'images'
            dummy_files = [f for f in os.listdir(dummy_path) if f.lower().endswith(('.jpg', '.jpeg', '.png'))]
            if dummy_files:
                _ = model(str(dummy_path / dummy_files[0]), imgsz=args.imgsz, conf=args.conf, device=args.device, verbose=False)
    ...
    for img_name in test_images:
        ...
        # Run inference with same parameters as testing_evaluation.py
        t_inference_start = time.perf_counter()
        results = model(str(img_path), imgsz=args.imgsz, conf=args.conf, device=args.device, verbose=args.verbose)
        ...
        for result in results:
            ...
            if result.keypoints is not None:
                ...
                try:
                    ...
                    # Start post-processing timer (computation only, excludes I/O)
                    t_postproc_start = time.perf_counter()

                    debug_data = []  # Collect debug info for later I/O
                    for i in range(xy_np.shape[0]):
                        ...
                        if xy_np.shape[1] >= 3:
                            ...
                            heading_v1 = calculate_heading_from_base_perpendicular(xy_np[i, 0], xy_np[i, 1], xy_np[i, 2])
                            ...
                            # Prepare overlay for v2 (computation only, don't save yet)
                            ...
                            # Store data for later GT comparison and JSON record (outside timing)
                            debug_data.append({
                                "pose_idx": i,
                                "overlay": overlay.copy(),
                                "dbg": dbg,
                                "img_name": img_name,
                                "heading_v1": heading_v1,
                                "heading_v2": heading_v2,
                                "kp": (xy_np[i, 0], xy_np[i, 1], xy_np[i, 2])
                            })
                    ...
                    # End post-processing timer (computation only)
                    t_postproc_end = time.perf_counter()
                    postproc_time = t_postproc_end - t_postproc_start
                    
                    # Ground truth comparisons and JSON record creation (OUTSIDE timing)
                    for item in debug_data:
                        pose_idx = item["pose_idx"]
                        heading_v1 = item["heading_v1"]
                        heading_v2 = item["heading_v2"]
                        
                        # Compare against GT (same pose index) if available
                        if gt_poses and pose_idx < len(gt_poses) and len(gt_poses[pose_idx]) >= 3:
                            g0, g1, g2 = gt_poses[pose_idx][0], gt_poses[pose_idx][1], gt_poses[pose_idx][2]
        ...
        # End total timer (before file I/O)
        t_total_end = time.perf_counter()
        total_time = t_total_end - t_total_start

        # FILE I/O (outside timing measurements)
        # Plot and save annotated frame
        annotated_frame = result.plot()
        output_path = out_dir / f"{img_name}"
        cv2.imwrite(str(output_path), annotated_frame)
        print(f" Saved result: {output_path}")
        
        # Save debug data (outside timing)
        if result.keypoints is not None and 'debug_data' in locals():
            debug_dir = out_dir / "v2_debug"
            debug_dir.mkdir(parents=True, exist_ok=True)
            for item in debug_data:
                pose_idx = item["pose_idx"]
                ...
                dbg = item["dbg"]
                img_name_item = item["img_name"]
                
                cv2.imwrite(str(debug_dir / f"{img_name_item.replace('.jpg','')}_pose{pose_idx}_v2_overlay.jpg"), overlay)
                
                if dbg is not None:
                    if dbg.get("roi") is not None:
                        cv2.imwrite(str(debug_dir / f"{img_name_item.replace('.jpg','')}_pose{pose_idx}_roi.jpg"), dbg["roi"])
                    if dbg.get("mask") is not None:
                        cv2.imwrite(str(debug_dir / f"{img_name_item.replace('.jpg','')}_pose{pose_idx}_mask.png"), dbg["mask"])

        # Record timings
        inference_times.append(inference_time)
        postprocessing_times.append(postproc_time)
        total_times.append(total_time)

        print(f"   Timing for {img_name} (heading computation only):")
        print(f"     Inference:       {inference_time*1000:8.2f} ms")
        print(f"     Post-processing: {postproc_time*1000:8.2f} ms")
    ...
    print("\n" + "="*72)
    print("TIMING SUMMARY (computation only, excluding GT & JSON & file I/O)")
    print("="*72)
    print(f"Inference configuration: imgsz={args.imgsz}, conf={args.conf}, device={args.device}, verbose={args.verbose}")
    print("="*72)
    if inference_times:
        print(f"\nInference times ({len(inference_times)} images, imgsz={args.imgsz}, conf={args.conf}):")
        print(f"  Mean:   {statistics.mean(inference_times)*1000:8.2f} ms")
        print(f"  Median: {statistics.median(inference_times)*1000:8.2f} ms")
        print(f"  Min:    {min(inference_times)*1000:8.2f} ms")
        print(f"  Max:    {max(inference_times)*1000:8.2f} ms")
        if len(inference_times) > 1:
            print(f"  Std:    {statistics.stdev(inference_times)*1000:8.2f} ms")

    if postprocessing_times:
        print(f"\nPost-processing times ({len(postprocessing_times)} images, heading computation only):")
        print(f"  Mean:   {statistics.mean(postprocessing_times)*1000:8.2f} ms")
        print(f"  Median: {statistics.median(postprocessing_times)*1000:8.2f} ms")
        print(f"  Min:    {min(postprocessing_times)*1000:8.2f} ms")
        print(f"  Max:    {max(postprocessing_times)*1000:8.2f} ms")
        if len(postprocessing_times) > 1:
            print(f"  Std:    {statistics.stdev(postprocessing_times)*1000:8.2f} ms")

    if total_times:
        print(f"\nTotal times per image ({len(total_times)} images, excluding GT & JSON & file I/O):")
        print(f"  Mean:   {statistics.mean(total_times)*1000:8.2f} ms")
        print(f"  Median: {statistics.median(total_times)*1000:8.2f} ms")
        print(f"  Min:    {min(total_times)*1000:8.2f} ms")
        print(f"  Max:    {max(total_times)*1000:8.2f} ms")
        if len(total_times) > 1:
            print(f"  Std:    {statistics.stdev(total_times)*1000:8.2f} ms")
